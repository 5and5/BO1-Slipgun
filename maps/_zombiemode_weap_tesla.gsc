...
init()
{
	...
	set_zombie_var( "tesla_max_enemies_killed_upgraded", 10 );
...
explode_to_near_zombies( player )
{
	enemies = get_array_of_closest(self.origin, GetAiSpeciesArray( "axis", "all" ));
	...
	{	
		enemy = enemies[ i ];
		if ( distancesquared( enemy.origin, self.origin ) < rsquared )
		{
	...
	for( i=0; i < marked_zombies.size; i++ )
	{	
		enemy = marked_zombies[i];
		if ( isalive( enemy ) )
		{
...
slipgun_play_arc_fx( target )
{
	...
	distance_squared = level.zombie_vars["slipgun_chain_radius"] * level.zombie_vars["slipgun_chain_radius"];
...
is_tesla_damage( mod )
{
	return ( ( IsDefined( self.damageweapon ) && (self.damageweapon == "tesla_gun_zm" || self.damageweapon == "tesla_gun_upgraded_zm" || self.damageweapon == "tesla_gun_powerup_zm" || self.damageweapon == "tesla_gun_powerup_upgraded_zm" || self.damageweapon == "tesla_gun_dlc_zm") ) && ( mod == "MOD_PROJECTILE" || mod == "MOD_PROJECTILE_SPLASH" ) );
}
...
tesla_sound_thread()
{
	...
	for( ;; )
	{
		...
		if( ( result == "weapon_change" || result == "grenade_fire" ) && (self GetCurrentWeapon() == "tesla_gun_zm" || self GetCurrentWeapon() == "tesla_gun_upgraded_zm" || self GetCurrentWeapon() == "tesla_gun_powerup_zm" || self GetCurrentWeapon() == "tesla_gun_powerup_upgraded_zm" || self GetCurrentWeapon() == "tesla_gun_dlc_zm") )
		{
...
tesla_pvp_thread()
{
	...
	for( ;; )
	{
		...
		if ( weapon != "tesla_gun_zm" && weapon != "tesla_gun_upgraded_zm" && weapon != "tesla_gun_powerup_zm" && weapon != "tesla_gun_powerup_upgraded_zm" || weapon != "tesla_gun_powerup_upgraded_zm" || weapon != "tesla_gun_dlc_zm")
		{
			continue;
		}
